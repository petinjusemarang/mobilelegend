...
local function runAI(root)
    ...
    activeHeartbeat = RunService.Heartbeat:Connect(function(dt)
        ...
        if _G.triggerCoast and not isFinishing then
            if not _G.coastingTimer then
                _G.coastingTimer = os.clock()
                _G.coastDelay = math.random(20, 40) / 10 -- 2.0 sampai 4.0 detik
            end

            if os.clock() - _G.coastingTimer > _G.coastDelay then
                isFinishing = true
                finishTime = os.clock()
            else
                -- Fase Cooldown (Motor melambat pelan-pelan ke 0 sebelum puter balik)
                speed = speed * 0.95
        ...
        if isFinishing then
            ...
            -- Speed: rem lebih kuat kalau dekat start biar ga bablas
            local returnSpeed = 80
            if distToStart < 80 then
                returnSpeed = math.max(10, distToStart * 0.6)
            end
            if elapsed < 1.5 then
                returnSpeed = speed + (returnSpeed - speed) * math.clamp(elapsed / 1.5, 0, 1)
            end
            -- Lerp speed lebih agresif turunnya
